...
Use `RTerm::Addon::RasterRenderer` when a Ruby-side pixel frame is needed. It renders cells into an RGBA buffer, draws built-in bitmap text masks, advances cursor blink state, composites decoded Sixel, PNG, GIF, and baseline JPEG images, presents unsupported JPEG frame data as a deterministic raster preview, and can export a PPM image for tooling.

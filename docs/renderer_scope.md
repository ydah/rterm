...
Use `RTerm::BrowserAdapter` to serve the bundled browser-side JavaScript and CSS. The adapter connects to BrowserBridge WebSockets, renders `screen` commands into DOM rows and cells or an optional WebGL canvas, forwards keyboard, pointer, paste, composition, clipboard, and resize events, measures character cells with browser layout, loads registered web fonts, and reports WebGL context lifecycle events.

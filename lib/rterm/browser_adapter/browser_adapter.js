(function (global) {
  ...
  class RTermBrowserAdapter extends EventTarget {
    ...
    mount() {
      ...
      this.accessibilityNode = document.createElement("div");
      this.accessibilityNode.className = "rterm-browser-accessibility";
      this.accessibilityNode.setAttribute("role", "document");
      ...
      const children = [this.screen, this.textarea, this.live, this.accessibilityNode, this.measureNode];
    ...
    connect(urlOrSocket) {
      ...
      this.socket.addEventListener("open", () => {
        ...
        if (this.sessionId) {
          ...
          this.send("create_session", this.createSessionPayload());
        }
      });
    ...
    createSessionPayload() {
      const payload = {
        cols: this.cols,
        rows: this.rows,
        terminalOptions: this.options.terminalOptions || {}
      };
      if (this.options.renderer) payload.browserRenderer = this.options.renderer;
      if (this.options.renderers) payload.renderers = this.options.renderers;
      if (this.options.raster === true) payload.raster = true;
      return payload;
    }
    ...
    applyCommand(command) {
      ...
      switch (command.type) {
        ...
        case "accessibility":
        case "screen_reader":
          this.updateAccessibility(payload || {});
          break;
    ...
    renderRaster(frame) {
      if (!frame) return;
      const rendered = this.renderRasterFrame(frame);
      this.dispatch("raster", { frame, rendered });
    }

    renderRasterFrame(frame) {
      if (this.webglRenderer && this.webglRenderer.renderRaster && this.webglRenderer.renderRaster(frame)) return true;

      return this.renderRasterToCanvas(frame);
    }

    renderRasterToCanvas(frame) {
      if (!this.rasterCanvasContext || !this.canvas || !Array.isArray(frame.pixels)) return false;
      ...
      const image = this.rasterCanvasContext.createImageData(width, height);
      let offset = 0;
      for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
          ...
          image.data[offset + 3] = pixel[3] == null ? 255 : pixel[3];
          offset += 4;
        }
      }
      this.rasterCanvasContext.putImageData(image, 0, 0);
      return true;
    }
    ...
    bindInput() {
      ...
      this.screen.addEventListener("mouseover", (event) => this.handleLinkHover(event));
      this.screen.addEventListener("mouseout", (event) => this.handleLinkLeave(event));
    ...
    handleClickSelection(event) {
      ...
      if (link) {
        event.preventDefault();
        this.sendLinkEvent("link_activate", link);
        this.dispatch("linkactivate", { uri: link.dataset.linkUri, event });
        return;
      }
    ...
    handleLinkHover(event) {
      const link = event.target.closest(".rterm-browser-cell.is-link");
      if (!link || link === this.hoveredLink) return;

      this.hoveredLink = link;
      this.sendLinkEvent("link_hover", link);
      this.dispatch("linkhover", { uri: link.dataset.linkUri, event });
    }
    ...
    handleLinkLeave(event) {
      ...
      this.sendLinkEvent("link_leave", link);
      this.dispatch("linkleave", { uri: link.dataset.linkUri, event });
    }

    sendLinkEvent(type, link) {
      this.sendHostEvent({
        type,
        uri: link.dataset.linkUri,
        row: Number(link.parentElement.dataset.row || 0),
        col: Number(link.dataset.col || 0)
      });
    }
    ...
    bindRasterCanvas(canvas) {
      if (!canvas.getContext) return false;

      this.rasterCanvasContext = canvas.getContext("2d", { alpha: true });
      if (!this.rasterCanvasContext) return false;

      this.element.classList.add("is-canvas");
      return true;
    }
    ...
    webglRequested() {
      return ["webgl", "raster", "hybrid"].includes(this.options.renderer) ||
    ...
    updateAccessibility(payload) {
      this.updateLiveRegion(payload);
      const tree = payload.tree || payload.accessibility_tree || payload.accessibilityTree;
      if (tree) {
        this.accessibilityNode.replaceChildren(this.renderAccessibilityNode(tree));
      } else if (payload.text) {
        this.accessibilityNode.textContent = payload.text;
      }
      this.dispatch("accessibility", payload);
    }

    renderAccessibilityNode(node) {
      const element = document.createElement("div");
      if (node.role) element.setAttribute("role", node.role);
      if (node.label || node.name) element.setAttribute("aria-label", node.label || node.name);
      if (node.row != null) element.dataset.row = String(node.row);
      if (node.col != null) element.dataset.col = String(node.col);
      if (node.text) element.textContent = node.text;
      (node.children || []).forEach((child) => element.appendChild(this.renderAccessibilityNode(child)));
      return element;
    }
    ...
    refresh() {
      if (this.lastScreen) this.renderScreen(this.lastScreen);
      return this;
    }
    ...
    setOption(name, value) {
      ...
      if (name === "cols") this.cols = value;
      if (name === "rows") this.rows = value;
      if (name === "renderer" || name === "raster" || name === "webgl") this.bindRendererSurface();
      return this;
    }

(function (global) {
  ...
  class RTermWebGLRenderer {
    ...
    render(screen, cell, style = {}) {
      ...
      screen.rows.forEach((row) => this.drawRow(row, cell, style));
      return true;
    }
    ...
    drawRow(row, cell, style) {
      (row.cells || []).forEach((item) => this.drawCell(item, row.row || 0, cell, style));
    }

    drawCell(cell, row, metrics, style) {
      ...
      this.drawRect(x, y, width, height, colors.background || style.backgroundColor || "#0b0f14");
      if (!cell.char || cell.char === " ") return;

      const attrs = cell.attributes || {};
      const glyph = this.glyph(cell.char, metrics, attrs, style);
      this.drawGlyph(x, y, width, height, colors.foreground || style.color || "#e5e7eb", glyph);
      if (attrs.underline) this.drawRect(x, y + height - 2, width, 1, colors.foreground || style.color || "#e5e7eb");
    }
    ...
    createProgram() {
      ...
      const fragment = this.compile(this.gl.FRAGMENT_SHADER, `
        ...
        void main() {
          ...
          gl_FragColor = vec4(u_color.rgb, u_color.a * sample.a);
        }
      `);

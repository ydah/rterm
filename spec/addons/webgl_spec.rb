...
RSpec.describe RTerm::Addon::WebGL do
  it "activates with renderer capabilities and emits change events" do
    ...
    expect(addon.rendererType).to eq(:webgl)
  ...
  it "clears texture atlas through terminal events" do
    terminal = RTerm::Terminal.new
    addon = described_class.new
    seen = []
    terminal_events = []

    addon.onTextureAtlasClear { |payload| seen << payload }
    terminal.on(:webgl_texture_atlas_clear) { |payload| terminal_events << payload }
    terminal.load_addon(addon)

    expect(addon.clearTextureAtlas).to be true
    expect(terminal.clearTextureAtlas).to be true

    expect(addon.state[:texture_atlas_clears]).to eq(2)
